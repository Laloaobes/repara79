...
const NotificationMenu = () => {
  ...
  return <div ref={rootRef} className="relative">
    <button ref={triggerRef} onClick={() => { setOpen(!open); if (!open) load(); }} aria-label={`Notificaciones, ${unread} sin leer`} aria-expanded={open} aria-controls="panel-notificaciones" className={`p-2 rounded-xl relative ${open ? 'bg-green-50 text-[#2d6a4f]' : 'text-slate-500 hover:bg-slate-100'}`}><Bell size={22} />{unread > 0 && <span className="absolute -right-1 -top-1 min-w-5 h-5 px-1 rounded-full bg-red-500 text-white text-[10px] font-black grid place-items-center">{unread > 99 ? '99+' : unread}</span>}</button>
    {open && <div id="panel-notificaciones" role="dialog" aria-label="Notificaciones" className="absolute right-0 mt-2 w-[min(22rem,85vw)] max-h-96 overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-xl z-50">
