...
const RepairsPage = () => {
  ...
  const startDialogRef = useRef<HTMLFormElement>(null);
  const initialStateRef = useRef<HTMLTextAreaElement>(null);

  useFocusTrap({
    active: selectedTicket !== null,
    containerRef: startDialogRef,
    initialFocusRef: initialStateRef,
    onEscape: () => { if (busy === null) setSelectedTicket(null); },
  });
  ...
  return <div className="mx-auto max-w-7xl space-y-7 p-4 md:p-8">
    {selectedTicket && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4 backdrop-blur-sm"><form ref={startDialogRef} role="dialog" aria-modal="true" aria-labelledby="tomar-reparacion-titulo" tabIndex={-1} onSubmit={start} className="w-full max-w-lg space-y-5 rounded-2xl bg-white p-6 shadow-2xl">
      <div className="flex items-start justify-between gap-4"><div><span className="text-xs font-black uppercase text-emerald-700">TK-{String(selectedTicket.id).padStart(3, '0')}</span><h2 id="tomar-reparacion-titulo" className="text-xl font-extrabold text-slate-800">Tomar reparación</h2><p className="mt-1 text-sm text-slate-500">{selectedTicket.title}</p></div><button type="button" onClick={() => setSelectedTicket(null)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100" aria-label="Cerrar"><X size={20} /></button></div>
